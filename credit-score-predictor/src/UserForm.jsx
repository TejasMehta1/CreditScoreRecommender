...
class userForm extends Component {
  ...
  handleCardChange = (e) => {
    if (
      ...
      credit_cards[e.target.dataset.id][
        e.target.className
      ] = e.target.value.toUpperCase();
      this.setState({ credit_cards }, () =>
        console.log(this.state.credit_cards)
      );
    } else {
    ...
    return (
      <>
        <div className="form-title">
          Enter your information the best you can:
        </div>
        <form
        ...
        >
          ...
          {credit_cards.map((val, idx) => {
            let fourdigitsid = `four-digits-${idx}`,
              limitid = `limit-${idx}`,
              spendingid = `spending-${idx}`,
              accuracyvalueid = `accuracy_value-${idx}`,
              paymentdeadlineid = `payment_deadline-${idx}`;
            return (
              <div key={idx}>
                <h4>{`Card #${idx + 1}`}</h4>
                ...
                <div className="a-question">
                  <label htmlFor={accuracyvalueid}>
                    Have you paid off most of your past balances on time? (y/n)
                  </label>
                  <input
                    type="text"
                ...
                <div className="a-question">
                  <label htmlFor={paymentdeadlineid}>
                    When is your next balance statement payment due? (Month Day,
                    Year)
                  </label>
                  <input
                    type="text"
          ...
          <button onClick={this.addLoan}>Add a loan</button>
          {credit_cards.map((val, idx) => {
            let loantypeid = `loan_type-${idx}`,
              accuracyvalueid = `accuracy_value-${idx}`,
              cardid = `card_id-${idx}`;
            return (
              <div key={idx}>
                ...
                <div className="a-question">
                  <label htmlFor={cardid}>
                    If you answered "Credit" to the previous question, enter the
                ...
                <div className="a-question">
                  <label htmlFor={accuracyvalueid}>
                    Have you paid off most of your past loans on time? (y/n)
                  </label>
                  <input
                    type="text"
                    name={accuracyvalueid}
